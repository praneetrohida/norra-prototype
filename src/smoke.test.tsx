...
test("sign in, onboard, browse orders and complete fulfilment", async () => {
  ...
  expect(screen.getByText("Trends Boutique")).toBeTruthy();
...
test("create a priced order from catalog and freeform items", async () => {
  ...
  await user.click(within(dialog).getByText("Rohan Mehta"));

  // add a catalog item via suggestion
  await user.type(screen.getByLabelText(/add item/i), "denim");
  await user.click(await screen.findByText("Slim Fit Denim Jeans"));
  // add a freeform item
  await user.type(screen.getByLabelText(/add item/i), "Garment Covers");
  await user.click(await screen.findByText(/add "Garment Covers" as a new item/i));
  ...
  expect(screen.getByText("Slim Fit Denim Jeans")).toBeTruthy();
  expect(screen.getByText("Garment Covers")).toBeTruthy();
...
test("customers and catalog CRUD screens render", async () => {
  ...
  await user.click(screen.getByText("StyleHub Retail"));
  await screen.findByText(/bulk orders around wedding and festival season/i);
  await user.click(screen.getByRole("button", { name: /go back/i }));
  await screen.findByRole("heading", { name: /^customers$/i });

  await user.click(screen.getByRole("link", { name: /catalog/i }));
  await screen.findByRole("heading", { name: /^catalog$/i });
  expect(screen.getByText("Cotton T-Shirt (Crew Neck)")).toBeTruthy();
